...
TokenHandler.prototype.handleGrantType = function(request, client) {
  var grantType = request.body.grant_type;
  var expiresIn = request.body.expires_in;
  ...
  if (expiresIn && (expiresIn.toString() !== parseInt(expiresIn, 10).toString())) {
    throw new InvalidRequestError('Invalid parameter: `expires_in`');
  }
  ...
  var accessTokenLifetime = this.getAccessTokenLifetime(client, expiresIn);
  var refreshTokenLifetime = this.getRefreshTokenLifetime(client, expiresIn);
...
TokenHandler.prototype.getAccessTokenLifetime = function(client, expiresIn) {
  return expiresIn || client.accessTokenLifetime || this.accessTokenLifetime;
};
...
TokenHandler.prototype.getRefreshTokenLifetime = function(client, expiresIn) {
  return expiresIn || client.refreshTokenLifetime || this.refreshTokenLifetime;
};
